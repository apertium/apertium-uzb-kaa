...
if test -t 0; then
    echo "Usage:" 1>&2
    echo "\$ bzcat uzbcorpus.txt.bz2 | ./$0 uzb-kaa" 1>&2
    exit 1
fi
...
covgoal="95.0" # percent goal we aim for
...
### Do the translation:
apertium-destxt | apertium -f none -d ../../ "$1"-postchunk | apertium-cleanstream -n | tee "$transfout" | lt-proc -g ../../"$1".autogen.bin | lt-proc -p ../../"$1".autopgen.bin > "$genout"
